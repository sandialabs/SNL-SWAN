...
!***********************************************************************
      ...
      USE Interp_v1  ! AJP Transmission coefficient determination
...
!       default transmission coefficient
        ...
        END IF
!*******************************************************************************
!   AJP      compute peak frequency of incoming wave
!   Earlier computation of peak freq
          EMAX = 0.
          ISIGM = -1
          DO IS = 1, MSC
             ETD = 0.
          ...
          TP=2.*PI/SPCSIG(ISIGM)  ! Tp Peak Period
!********************************************************************************
!
!       incoming wave height  HSIN =significant wave height
        ...
        IF (ITRAS .EQ. 0) THEN
!*********************************************************************************
! AJP Calling Power Matrix Subroutine
      print*,case_n ! option for power model
      if (case_n.NE.0) then   ! not equal to zero uses one of the modified options
        call P_Matrix(TP,HSIN,RCW)  ! Call power matrix subroutine for RCW values
        print*,RCW
! Compute incident wave power
      POWER=0.42*HSIN*HSIN*TP ! debugging print statments for relevant variables
        print*,TP
        print*,HSIN
        print*,POWER
! Use proportion of power taken off to estimate transmission coefficient
! OBHKT - actual transmission coefficient used by SWAN
        if (RCW.EQ.0.0) then
        OBHKT = 1.0
        else
        OBHKT = 1-RCW
        ...
        print*,OBHKT
        WRITE (PRINTF,*) X1,Y1,OBHKT  ! Writes transmission coefficient (Kt) to main output file
      endif
!***********************************************************************************        
!       constant transmission coefficient
!         User defined transmission coefficient concerns ratio of         40.09
!         waveheights, so:                                                40.09
      if (case_n.EQ.0) then  !standard transmission coefficient squared (Kt^2)
          OBHKT = COBST%TRCOEF(1)                                         40.31
          TRCF  = OBHKT * OBHKT                                           40.09
! If using power as ratio do not use square
      else
          TRCF  = OBHKT  ! Ktp Not squared for power matrix model
      endif
